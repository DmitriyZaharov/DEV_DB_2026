...
INSERT INTO Orders (order_id, order_number, order_date, signer_name) VALUES 
(101, 'П-001', '2023-01-10', 'Иванов И.И.'), -- Прием на работу
(102, 'Т-015', '2024-02-01', 'Иванов И.И.'), -- Перевод на другую должность
(103, 'У-005', '2025-03-15', 'Петров П.П.'); -- Увольнение

-- 2. ПРИЕМ НОВОГО СОТРУДНИКА
-- Сначала создаем запись в справочнике сотрудников
INSERT INTO Employees (
    employee_id, last_name, first_name, birth_date, reg_address, 
    hire_date, hire_order_id, email
) VALUES (
    50, 'Сидоров', 'Алексей', '1995-05-20', 'г. Москва, ул. Ленина, 1', 
    '2023-01-15', 101, 'sidorov@company.com'
);

-- Назначаем на должность в штатную расстановку (отдел 1, должность 2)
INSERT INTO Staffing (
    staffing_id, employee_id, dept_id, position_id, 
    employment_rate, actual_salary, start_date, appointment_order_id
) VALUES (
    500, 50, 1, 2, 1.0, 75000.00, '2023-01-15', 101
);

-- 3. ПЕРЕВОД СОТРУДНИКА (Смена должности/отдела)
-- Закрываем старую запись в расстановке
UPDATE Staffing 
SET end_date = '2024-01-31', 
    release_order_id = 102
WHERE staffing_id = 500;

-- Открываем новую запись (например, повышение зарплаты и смена отдела)
INSERT INTO Staffing (
    staffing_id, employee_id, dept_id, position_id, 
    employment_rate, actual_salary, start_date, appointment_order_id
) VALUES (
    501, 50, 2, 2, 1.0, 95000.00, '2024-02-01', 102
);

-- 4. УВОЛЬНЕНИЕ
-- Обновляем основную карточку сотрудника
UPDATE Employees 
SET fire_date = '2025-03-15', 
    fire_order_id = 103
WHERE employee_id = 50;

-- Закрываем текущую активную запись в штатной расстановке
UPDATE Staffing 
SET end_date = '2025-03-15', 
