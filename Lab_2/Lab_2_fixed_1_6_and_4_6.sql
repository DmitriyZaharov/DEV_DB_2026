...
--SELECT * 
--FROM "Sales"."Customers"
--WHERE contactname !~* 'Ann|Linda|Robert';
---- Использую регулярные выражения.
---- Исключаем любые вариации написания (Ann, ann, ANN), используя оператор !~* 

-- В нашем случае contactname = Petrov, Ivan имеет формат "Фамилия, Имя"
-- простым решением будет поиск запрещенных имен именно в конце строки после разделителя.
SELECT * 
FROM "Sales"."Customers"
WHERE contactname !~* ',\s*(Ann|Linda|Robert)$';
--	, — гарантирует, что мы смотрим на часть строки с именем, а не фамилией.
--	\s* — игнорирует пробел после запятой (если он есть).
--	(Ann|Linda|Robert) — список имен для исключения.
--	$ — якорь конца строки. Он гарантирует, что мы исключаем именно имя Ann, но оставляем Annabelle или Ann-Marie.
--	!~* — исключает совпадения без учета регистра.
...
SELECT 
    ...
    split_part(contactname, ', ', 2) AS first_name
FROM "Sales"."Customers"
WHERE contactname ~ '^[SK].*, [PM]'
ORDER BY last_name;
-- Использую регулярные выражения.
-- ^ — привязка к началу строки.
-- [SK] — первая буква фамилии должна быть либо S, либо K.
--.* — любое количество любых символов (остаток фамилии).
--, — обязательное наличие запятой и пробела (разделитель).
-- [PM] — первая буква имени после пробела должна быть либо P, либо M.
...
--SELECT 
--    orderid AS "Номер заказа",
--    -- Считаем в numeric, округляем, и только в конце превращаем в money
--    ROUND(SUM(unitprice * qty * (1 - discount))::numeric, 2)::money AS "Общая стоимость"
--FROM "Sales"."OrderDetails"
--GROUP BY orderid
---- Сравниваем сумму (число) с числом 1000
--HAVING SUM(unitprice * qty * (1 - discount)) > 1000
--ORDER BY SUM(unitprice * qty * (1 - discount)) DESC;

--	В нашем случае unitprice имеет тип money
--	Главная проблема здесь: результат умножения money * float (скидка) или money * int 
--	все равно остается типом money, который нельзя сравнивать с числом 1000.
