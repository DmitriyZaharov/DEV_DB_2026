...
--		Индексы JSON: Начинаются с 0.
--		Пояснения по операторам:
--		-> - возвращает JSON объект/массив
--		->> - возвращает значение как текст
--		@> - проверяет, содержит ли JSON указанный элемент
--		jsonb_set - функция для изменения значения в JSONB
...
SELECT 
    -- 1. Список офисов: используем data->'offices' для получения массива офисов
    data->'offices' AS offices,
    -- 2. Название компании: используем оператор ->> для получения значения как текста
    data->>'company_name' AS company,
    -- 3. Первый офис: обращаемся по индексу 0 массива offices
    data->'offices'->0 AS first_office,
    -- 4. Имя второго офиса: берем элемент с индексом 1 и извлекаем поле name
    data->'offices'->1->>'name' AS second_office_name,
    -- 5. Проверка Питера. Возвращает true если есть офис в СПб, иначе false
--    вариант_1:@> - проверяет, содержит ли JSON указанный элемент
    data->'offices' @> '[{"city": "Санкт-Петербург"}]' AS has_spb,
--    вариант_2:  используем jsonb_array_elements для преобразования массива в набор 
--	  строк и проверяем наличие нужного города   
    EXISTS (
    	SELECT 1
        FROM jsonb_array_elements(data->'offices') AS office
        WHERE office->>'city' = 'Санкт-Петербург'
    ) AS has_spb_office, 
-- 		6. Замена Парижа на Питер:используем функцию jsonb_set, которая позволяет 
--    изменить значение по указанному пути ({offices,0,city})
    jsonb_set(data, '{offices, 0, city}', '"Санкт-Петербург"') AS updated_json
FROM json_data;
